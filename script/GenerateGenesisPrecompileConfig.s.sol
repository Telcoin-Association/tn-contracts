...
contract GenerateGenesisPrecompileConfig is GenesisPrecompiler, Script {
    ...
    /// @dev Writes the `nonce`/`balance` pair shared by every predeploy whose entry is
    /// hardcoded rather than produced by `yamlAppendGenesisAccount`. Kept on `PREDEPLOY_NONCE`
    /// so the whole file states one convention; the deployer-EOA entries below deliberately
    /// hardcode their own `nonce: 1`, which means "presigned tx spent", not "EIP-161 initial"
    function _writePredeployNonceAndBalance() internal {
        vm.writeLine(dest, string.concat("  nonce: ", vm.toString(uint256(PREDEPLOY_NONCE))));
        vm.writeLine(dest, "  balance: 0");
    }
    ...
    function instantiateMulticall3() internal {
        ...
        _writePredeployNonceAndBalance();
        vm.writeLine(
            dest,
