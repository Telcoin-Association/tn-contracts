...
- **Stake Versions**: Records are kept of validators joining under different versions for accurate stake tracking and weighted reward calculation. Stake versions are set on a per-validator basis at stake time and may be changed in-place via `requestStakeVersionChange`. Changes are version-forward only; moving to an earlier version index is not supported (though a later version may carry a lower stake amount).
  - **Epoch-Boundary Version Queue**: For in-service validators (PendingActivation, Active), a version change request only enters a queue; the change is applied automatically inside `concludeEpoch` at an epoch boundary, after that boundary's slashes have landed. Validators have no control over settlement timing, so no request or settlement timing can move value ahead of a slash, and reward weights are stable within an epoch by construction (versions never change mid-epoch for in-service validators).
    ...
    - *Settlement gas bound*: the queue processes one entry per in-service validator inside `concludeEpoch`, the same iteration bound `_updateValidatorQueue` carries, at roughly 30k gas per settling entry (pure storage; refunds accrue as `claimRefund` credits, no external calls). The worst case is a single boundary where every in-service validator's decrease ages out at once: `ConcludeEpochGasBench` measures 33.9M gas for a 1000-entry wave against the client's 100M per-call system-call budget (`SYSTEM_CALL_GAS_LIMIT`), leaving headroom to roughly 2,900 entries. For reference, under the earlier 30M budget the ceiling was near 860 entries (a 700-entry wave measured ~25.4M). Since the in-service set is governance-gated (each validator needs a minted ConsensusNFT), this bound is reached only by a coordinated wave, not a permissionless input. If the validator ceiling is ever raised toward that limit, the settlement loop can be paginated with a rotating cursor - rolled entries fail safe by staying at the higher-collateral version another epoch - or resolved lazily: an `_effectiveStakeVersion(validator)` view could return `pending.targetVersion` once `currentEpoch >= pending.requestEpoch + STAKE_DECREASE_DELAY_EPOCHS` and fall back to the stored version otherwise, removing the boundary loop entirely at the cost of routing every reward and slashing path through the resolved version and settling each validator's balance and refund in a separate transaction.
