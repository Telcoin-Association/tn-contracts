...
- the version-queue settlement inside concludeEpoch is bounded by the in-service validator count (one entry per validator, ~30k gas each); worst case is ~33.9M for a 1000-entry wave against the 100M per-call budget (ceiling ~2,900 entries), governance-gated so only reachable by a coordinated wave; pagination or lazy version resolution are the mitigations if the validator ceiling ever approaches it
